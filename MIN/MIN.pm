...
This class implements the C<MIN> element from the C<VOTABLE> DTD. This
element is used to store minimum values for a numeric quantity.

The C<MIN> element is a Tier 0 element, and is described by the
...
Create a new C<VOTABLE::MIN> object, and return a reference to it. If
the first argument (C<$str_or_ref>) is a string, it is used as the
initial C<PCDATA> content of the C<MIN> element. If the first argument
is a reference to a C<XML::DOM::Element> object, that object is used
to initialize the new C<MIN> element (implicitly assuming that the
C<XML::DOM::Element> object contains a valid C<MIN> element). The
...
and return an empty C<VOTABLE::MIN> object. Return C<undef> if an
error occurs.

=head3 C<get_inclusive>

Return the value of the C<inclusive> attribute. Return C<undef> if the
attribute has not been set, or an error occurs.

=head3 C<set_inclusive($inclusive)>

Set the value of the C<inclusive> attribute to the specified
...
Return a string containing the C<PCDATA> content of the C<MIN>
element. Return C<undef> if the element has no C<PCDATA> content, or
an error occurs.

=head3 C<set($str)>

Set the C<PCDATA> content of the C<MIN> element to the specified
...
C<VOTABLE::MIN> object I<always> has an underlying
...
# $Log: MIN.pm,v $
# Revision 1.1.1.8  2002/05/21  14:11:48  elwinter
# Incremented $VERSION to 0.02.
#
# Revision 1.1.1.7  2002/05/21  13:45:42  elwinter
# Overhauled and updated documentation.
#
# Revision 1.1.1.6  2002/05/07  13:17:36  elwinter
# Minor type fixed.
#
# Revision 1.1.1.5  2002/05/07  13:12:59  elwinter
# Overhauled.
#
# Revision 1.1.1.4  2002/05/03  15:01:11  elwinter
# Overhauled.
#
# Revision 1.1.1.3  2002/05/01  16:36:24  elwinter
# Overhauled.
#
# Revision 1.1.1.2  2002/04/28  15:16:15  elwinter
...
# current class (VOTABLE::MIN), unless this constructor is called for
...
sub new()
{
    ...
    # Construct the VOTABLE::MIN object from the XML::DOM::Element
    # object.
...
{
    my($this) = @_;
    return($this->_get_XMLDOM->getAttribute('inclusive'));
}
...
sub set_inclusive()
{
	...
	carp("Invalid $TAG_NAME 'inclusive' attribute value: $inclusive!");
	return(undef);
    }
    $this->_get_XMLDOM->setAttribute('inclusive', $inclusive);
...
# of their arguments, the VOTABLE::MIN object and the underlying
...
# This internal method is used to assemble a VOTABLE::MIN object from
# a XML::DOM::Element object. Return 1 on success, or 0 on failure.

# Since the MIN element is a Tier 0 element, there is nothing for this
