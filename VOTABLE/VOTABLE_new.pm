...
specified value. Valid ID attributes must start with a letter, and
contain only alphanumeric characters and underscores. Throw an
exception if an error occurs.

=head3 remove_ID()

Remove the the 'ID' attribute (if any) for this VOTABLE element. Throw
...
the specified value. Currently, only '1.0' is supported as the value
of the 'version' attribute. Throw an exception if an error occurs.

=head3 remove_version()

Remove the the 'version' attribute (if any) for this VOTABLE
...
DESCRIPTION element child of this VOTABLE element. Any existing
DESCRIPTION element in this VOTABLE element is replaced with the new
DESCRIPTION. Throw an exception if an error occurs.

=head3 append_DESCRIPTION($description)

Use $description (a VOTable::DESCRIPTION object, or a
XML::LibXML::Element object for a DESCRIPTION element) to append the
DESCRIPTION element child of this VOTABLE element. Throw an exception
if an error occurs.
...
=head3 append_DEFINITIONS(@definitions)

Use @definitions (a list of a single VOTable::DEFINITIONS object) to
append the DEFINITIONS element child to this VOTABLE element. Any
existing DEFINITIONS element in this VOTABLE element is retained,
which is an error condition. Throw an exception if an error occurs.
...
$Id: VOTABLE.pm,v 1.1.1.11 2003/04/09 16:25:00 elwinter Exp elwinter $
...
sub set_ID()
{
    ...
    $id =~ /^[A-Za-z]/
	or croak('ID must start with a letter!');
    if ($id =~ /\W/) {
	croak('ID must contain only alphanumerics and underscores!');
    }
...
sub set_version()
{
    ...
    $version eq '1.0'
	or croak('Version must be 1.0!');

    # Set the version.
    $self->SUPER::set_version($version);
...
sub get_DESCRIPTION()
{
	...
	$description = new VOTable::DESCRIPTION $description or
...
sub set_DESCRIPTION()
{
    ...
    # Old DESCRIPTION element object.
    my($old_description);

    #--------------------------------------------------------------------------

    # Make sure this is a DESCRIPTION element.
    $self->nodeName eq 'DESCRIPTION' or
	croak('Not a DESCRIPTION element!');

    # If this VOTABLE element has a DESCRIPTION element, replace it
    # with the new DESCRIPTION. Otherwise, add the new DESCRIPTION as
    # the first child element of the VOTABLE.
    $old_description = $self->get_DESCRIPTION;
    if ($old_description) {
	$self->replaceChild($description, $old_description);
    } else {
    }

    # Make sure only one DESCRIPTION element is used.
    $self->SUPER::set_DESCRIPTION($description);
...
sub get_DEFINITIONS()
{
    ...
    if ($definitions and not $definitions->isa('VOTable::VOTABLE')) {
	$definitions = new VOTable::DEFINITIONS $definitions or
