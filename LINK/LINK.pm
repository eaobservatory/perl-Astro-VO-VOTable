...
This class implements the LINK element from the VOTABLE DTD.
...
$Id: LINK.pm,v 1.1.1.5 2002/05/07 13:37:09 elwinter Exp $
...
{
    my($this) = @_;
    return($this->{'XML::DOM::Element'}->getAttribute('action'));
}

sub set_action()
{
    my($this, $action) = @_;
    $this->{'XML::DOM::Element'}->setAttribute('action', $action);
...
{
    my($this) = @_;
    return($this->{'XML::DOM::Element'}->getAttribute('content-role'));
}
...
sub set_content_role()
{
	...
	carp("Invalid $TAG_NAME attribute value: $content_role!");
	return(undef);
    }
    $this->{'XML::DOM::Element'}->setAttribute('content-role', $content_role);
...
{
    my($this) = @_;
    return($this->{'XML::DOM::Element'}->getAttribute('content-type'));
}

sub set_content_type()
{
    my($this, $content_type) = @_;
    $this->{'XML::DOM::Element'}->setAttribute('content-type', $content_type);
...
{
    my($this) = @_;
    return($this->{'XML::DOM::Element'}->getAttribute('gref'));
}

sub set_gref()
{
    my($this, $gref) = @_;
    $this->{'XML::DOM::Element'}->setAttribute('gref', $gref);
...
{
    my($this) = @_;
    return($this->{'XML::DOM::Element'}->getAttribute('title'));
}

sub set_title()
{
    my($this, $title) = @_;
    $this->{'XML::DOM::Element'}->setAttribute('title', $title);
