...
This class implements the TABLEDATA element from the VOTABLE DTD.
...
$Id: TABLEDATA.pm,v 1.1.1.5 2002/05/14 17:43:00 elwinter Exp $
...
sub set_tr()
{
    ...
    # Attach each TR XML::DOM::Element to the XML::DOM::Document. THIS
    # IS IMPORTANT!
    foreach $votable_tr (@votable_tr) {
	$votable_tr->{'XML::DOM::Element'}->setOwnerDocument($xmldom_document);
    }
    ...
    foreach $votable_tr (@votable_tr) {
	$xmldom_element_this->
	    appendChild($votable_tr->{'XML::DOM::Element'});
    }
...
sub _build_from_XMLDOM()
{
    ...
    # There should be at least one TD, eventually.
    if (@xmldom_elements =
	$this_xmldom_element->getElementsByTagName('TR', 0)) {
