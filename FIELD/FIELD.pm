...
This class implements the C<FIELD> element from the C<VOTABLE>
DTD. This class stores metadata for a single field (column) in a
table.

The C<FIELD> element is a Tier 3 element, and is described by the
...
Create and return a new C<VOTABLE::FIELD> object, based on the
...
C<VOTABLE::FIELD> object. Return C<undef> if an error occurs.
...
element of this C<FIELD> element. Return C<undef> If no C<DESCRIPTION>
element is found, or an error occurs.

=head3 C<set_description($votable_description)>

Set the C<DESCRIPTION> element for this C<FIELD> element to the
...
which are the children of this C<FIELD> element. Return an empty list
if no C<LINK> elements are found, or if an error occurs.

=head3 C<set_link(@votable_link)>

Set the C<LINK> elements for this C<FIELD> element using the supplied
...
elements which are the children of this C<FIELD> element. Return an
empty list if no C<VALUES> elements are found, or if an error occurs.

=head3 C<set_values(@votable_values)>

Set the C<VALUES> elements for this C<FIELD> element using the
...
C<VOTABLE>, C<VOTABLE::DESCRIPTION>, C<VOTABLE::LINK>,
C<VOTABLE::TABLE>, C<VOTABLE::VALUES>
...
#
# Revision 1.1.1.7  2002/05/21  13:42:31  elwinter
...
sub new()
{
    ...
    # Construct the VOTABLE::FIELD object from the XML::DOM object.
